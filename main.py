...
class EditNutrients:
    ...
    def execute(self, barcode):
        ...
        print(
            f"현재 상품 정보: 제품명: {product_info[0]}, 탄수화물: {product_info[1]}, 단백질: {product_info[2]}, 지방: {product_info[3]}, 나트륨: {product_info[4]}"
        )

        new_product_info = input(
            "새로운 제품명, 탄수화물, 단백질, 지방, 나트륨의 값을 순서대로 입력하세요 (띄어쓰기로 구분):")
...
class ChoosePerson:
    ...
    def execute(self):
        while True:
            ...
            if choice == 1:
                profiles = [
                    f[:-4] for f in os.listdir('profile') if f.endswith('.txt')
                ]
                if not profiles:
                ...
                with open(os.path.join('profile', f"{profile_name}.txt"),
                          "r") as file:
                    profile_info = file.read().split(' ')

                print(
                    f"현재 프로필 정보: 키 {profile_info[0]}, 몸무게 {profile_info[1]}, 골격근량 {profile_info[2]}, 체지방률 {profile_info[3]}, 주당 운동 횟수 {profile_info[4]}, 운동 시간 {profile_info[5]}, 식단 균형 {profile_info[6]}"
                )
                ...
                while True:
                    ...
                    if menu_choice == 4:
                        ...
                        meal = '아침' if menu_choice == 1 else (
                            '점심' if menu_choice == 2 else '저녁')
                        FoodModificationForEachPartition().execute(
                            profile_name, meal)
                    elif menu_choice == 5:
                        break
                    else:
    ...
    def execute(self, profile_name, meal):
        ...
        while True:
            ...
            if choice == 1:
                ...
                with open(os.path.join(meal_folder, f"{meal}.txt"),
                          "a") as file:
                ...
                meal_file = os.path.join(meal_folder, f"{meal}.txt")
                if not os.path.exists(meal_file) or os.stat(
                        meal_file).st_size == 0:
                    print("먹은 식사가 없습니다.")
                else:
                    print("저장된 식사 정보:")
                    with open(os.path.join(meal_folder, f"{meal}.txt"),
                        ...
                        for i, meal_info in enumerate(meals, 1):
                            ...
                            total_nutrients = [
                                total_nutrients[j] + float(meal_info[j + 1])
                                for j in range(4)
                            ]
                            print(
                                f"{i}. 제품명: {meal_info[0]}, 탄수화물: {meal_info[1]}, 단백질: {meal_info[2]}, 지방: {meal_info[3]}, 나트륨: {meal_info[4]}"
                            )
                        print(
                            f"총합: 탄수화물: {total_nutrients[0]}, 단백질: {total_nutrients[1]}, 지방: {total_nutrients[2]}, 나트륨: {total_nutrients[3]}"
                        )
...
class EnterPhysicalInformation:
    ...
    def execute(self):
        ...
        physical_info_ment = [
            "키를 입력하세요:", "몸무게를 입력하세요:", "골격근량을 입력하세요:", "체지방률을 입력하세요:"
        ]
        physical_info = []
        for i in range(4):
            physical_info.append(input(physical_info_ment[i]))

        with open(os.path.join('profile', f"{profile_name}.txt"), "w") as file:
            file.write(" ".join(physical_info))
...
class EnterActivityInformation:
    ...
    def execute(self, profile_name):
        activity_info_ment = ["주당 운동 횟수를 입력하세요:", "운동 시간을 입력하세요:"]
        activity_info = []
        for i in range(2):
            activity_info.append(input(activity_info_ment[i]))

        with open(os.path.join('profile', f"{profile_name}.txt"), "a") as file:
            file.write(" " + " ".join(activity_info))
...
class EditPhysicalInformation:
    ...
    def execute(self, profile_name):
        ...
        print("\n============Edit Physical Info============\n")
        print(
            f"현재 프로필 정보: 키: {profile_info[0]}, 몸무게: {profile_info[1]}, 골격근량: {profile_info[2]}, 체지방률: {profile_info[3]}"
        )

        new_physical_info = input(
            "새로운 키, 몸무게, 골격근량, 체지방률의 값을 순서대로 입력하세요 (띄어쓰기로 구분):")

        try:
            with open(filename, "w") as file:
                file.write(new_physical_info + " " +
                           " ".join(profile_info[4:]))
...
class EditActivityInformation:
    ...
    def execute(self, profile_name):
        ...
        print("\n============Edit Activity Info============\n")
        print(
            f"현재 프로필 정보: 주당 운동 횟수: {profile_info[4]}, 운동 시간: {profile_info[5]}"
        )

        new_activity_info = input(
            "새로운 주당 운동 횟수와 운동 시간을 순서대로 입력하세요 (띄어쓰기로 구분):")

        try:
            with open(filename, "w") as file:
                file.write(" ".join(profile_info[:4]) + " " +
                           new_activity_info + " " + profile_info[6])
            print("프로필 정보가 성공적으로 수정되었습니다.")
        except Exception as e:
            print("파일 저장 중 오류가 발생했습니다.")
            print(e)
